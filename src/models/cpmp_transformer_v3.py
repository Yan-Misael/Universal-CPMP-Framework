...
class CPMPTransformer(Transformer):
    def __init__(self, H, X_dim, d_model=64, nhead=8, num_layers=2, ff_dim_multiplier=4, dropout=0.1):
        ...
        self.x_projection = nn.Linear(X_dim, d_model)  # Proyectamos X para que tenga sentido con d_model
        self.fusion_layer = nn.Linear(d_model * 2, d_model) # Fusionamos [Pila, X]
        self.fusion_norm = nn.LayerNorm(d_model) # Estabiliza la combinación
        
        # Capa de fusión si prefieres concatenar en lugar de sumar
    ...
    def forward(self, S, X):
        ...
        # --- PASO 5: Inter-Stack Attention ---
        x_global = self.inter_stack_attention(stack_embeddings)
        
        # --- PASO 6: Generar combinaciones (Sincronizado con Adapter) ---
        # Usamos un método determinista para que coincida con el Adapter
        ...
        # --- PASO 7: Cross Attention y Score ---
        attn_output, _ = self.cross_attention(query=x_origin, key=x_dest, value=x_dest)
        logits = self.score_head(attn_output).squeeze(-1) 

        # --- PASO 8: Máscara de Acciones Inválidas ---
        is_origin_empty = (S == -1).all(dim=2) 
        is_dest_full = (S != -1).all(dim=2) 
