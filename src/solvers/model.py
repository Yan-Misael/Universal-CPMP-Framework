...
class ModelSolver(Solver): 
    ...
    def solve_from_path(self, instance_path, H, max_steps):
        ...
        # EXTRAER S_MAX DIRECTAMENTE DEL ADAPTADOR DEL MODELO
        S_max = self.model.layout_adapter.S_max
        ...
        with torch.no_grad():
            while not layout.is_sorted():
                ...
                for i in range(len(top_indices)):
                    ...
                    src = int(best_index / (S_max - 1))
                    r = best_index % (S_max - 1)
                    dst = r if r < src else r + 1

                    if src >= len(layout.stacks) or dst >= len(layout.stacks):
                        continue
